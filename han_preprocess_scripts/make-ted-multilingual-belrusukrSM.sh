...
# Han: CHANGE ME!!!
RAW_DDIR=han_data/ted_raw_belrusukrSM_sepspm"$VOCAB_SIZE"/ # Han: change this for different data setup
PROC_DDIR=han_data/ted_processed/belrusukrSM_sepspm"$VOCAB_SIZE"/ # Han: change this for different data setup
BINARIZED_DDIR=fairseq/data-bin/ted_belrusukrSM_sepspm"$VOCAB_SIZE"/ # Han: change this for different data setup
...
for LAN in ${LANS[@]}; do
  ...
  for f in "$RAW_DDIR"/"$LAN"_eng/*.orig.{eng,$LAN} ; do
    ...
    if [ ! -f "$f1" ]; then
      echo "tokenize $f1..."
      sed -n '1,20000p' $f | perl $TOKENIZER > $f1 # control data size
    fi
  done
done

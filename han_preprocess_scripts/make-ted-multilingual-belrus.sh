...
# Han: CHANGE ME!!!
RAW_DDIR=han_data/ted_raw_belrus_sepspm"$VOCAB_SIZE"/ # Han: change this for different data setup
PROC_DDIR=han_data/ted_processed/belrus_sepspm"$VOCAB_SIZE"/ # Han: change this for different data setup
BINARIZED_DDIR=fairseq/data-bin/ted_belrus_sepspm"$VOCAB_SIZE"/ # Han: change this for different data setup
...
for LAN in ${LANS[@]}; do
  ...
  for f in "$RAW_DDIR"/"$LAN"_eng/*.orig.{eng,$LAN} ; do
    ...
    if [ ! -f "$f1" ]; then
      echo "tokenize $f1..."
      sed -n '1,40000p' $f | perl $TOKENIZER > $f1 # control data size
    fi
  done
done

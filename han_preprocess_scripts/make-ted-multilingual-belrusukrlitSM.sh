...
VOCAB_SIZE=8000
BASE_DDIR=han_data/ted_raw_base/

# Han: CHANGE ME!!!
RAW_DDIR=han_data/ted_raw_belrusukrlitSM_sepspm"$VOCAB_SIZE"/ # Han: change this for different data setup
PROC_DDIR=han_data/ted_processed/belrusukrlitSM_sepspm"$VOCAB_SIZE"/ # Han: change this for different data setup
BINARIZED_DDIR=fairseq/data-bin/ted_belrusukrlitSM_sepspm"$VOCAB_SIZE"/ # Han: change this for different data setup
...
# Han: clear existing preprocessed data
rm -rf "$RAW_DDIR"
rm -rf "$PROC_DDIR"
rm -rf "$BINARIZED_DDIR"
cp -r "$BASE_DDIR" "$RAW_DDIR"
...
for LAN in ${LANS[@]}; do
  ...
  for f in "$RAW_DDIR"/"$LAN"_eng/*.orig.{eng,$LAN} ; do
    ...
    if [ ! -f "$f1" ]; then
      echo "tokenize $f1..."
      sed -n '1,13333p' $f | perl $TOKENIZER > $f1 # control data size
    fi
  done
done
